...
module.exports.register = [
    ...
    validator.body('password', "please enter Passowrd").isLength({min:1}),
    ...
    function(req, res){
        ...
        var hash = bcrypt.hashSync(user.paswword, salt);
        user.paswword = hash
...
module.exports.login = [
    ...
    function(req, res){
        ...
        User.findOne({email: req.body.email}, function(err, user){
            ...
            return bcrypt.compare( req.body.password, user.paswword, function(err, isMatched){
                if(isMatched === true){
                    return res.json({
                        user: {
                            _id : user._id,
                        ...
                        token : jwt.sign({_id: user._id, email: user.email, full_name: user.full_name},
                                config.authSecret) // generate JWT token here
                    });
                }
                else{
